    ...
    <>
      ...
      <div className="sm:hidden fixed top-4 right-4 z-30">
        <button className="p-3 bg-gray-800 rounded-lg hover:bg-gray-700 transition-colors">
          <Menu onClick={() => setSidebarOpen(true)} className="w-5 h-5 text-gray-200"></Menu>
        </button>
      </div>
          ...
          {user ? (
                  ...
                  {isTyping ? (
                    <div className="flex items-center gap-2 text-sm">
                      <div className="flex gap-1">
                        <div style={{animation: "0.2s"}} className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce"></div>
                        <div style={{animation: "0.2s"}} className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce"></div>
                        <div style={{animation: "0.2s"}} className="w-1.5 h-1.5 bg-blue-500 rounded-full animate-bounce"></div>
                      </div>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500 mr-1">Choose a chat from the sidebar to start messaging</p>
              </div>
            </div>
          )}
        </div>
